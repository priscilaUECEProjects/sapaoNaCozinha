...
export default function PaginaLogin(){
    ...
    return(
        <div className='flex items-center justify-center w-full h-dvh bg-hunter-green-ligther bg-[url("./assets/img/fundo-verde.png")] bg-repeat bg-[auto_500px]'>
                <form onSubmit={handleLogin} method="post" className="bg-pearl w-xl p-4 text-[var(--color-dark-green)] shadow-[-10px_-10px_30px_4px,_10px_10px_30px_4px] shadow-dark-green rounded-2xl flex flex-col items-center gap-4">
                    <h1 className="font-bold text-3xl mb-5">Login</h1>

                    <label htmlFor="emailLogin" className='font-bold'>E-mail</label>
                    <input type="email" name="emailLogin" id="emailLogin" value={DadosLogin.emailLogin} onChange={handleChangeLogin} className="border-1 border-[var(--color-dark-green)] w-10/12 text-xl p-2 rounded" required/>

                    <label htmlFor="senhaLogin" className='font-bold'>Senha</label>
                    <input type="password" name="senhaLogin" id="senhaLogin" value={DadosLogin.senhaLogin} onChange={handleChangeLogin} className="border-1 border-[var(--color-dark-green)] w-10/12 text-xl p-2 rounded" required/>
