        ...
        <div className="flex items-center justify-center min-h-screen bg-[var(--color-quinacridone-magenta)] bg-[url('./assets/img/fundo-roxo.png')] bg-repeat bg-[size:500px]">
            <div className="text-[#55133b] font-bold"> 
                <form className="flex flex-col items-center gap-4 p-6 bg-[#e6e1c5] rounded-2xl shadow-[-10px_-10px_30px_4px,_10px_10px_30px_4px]" onSubmit={handleSubmit}>
                    ...
                    <input type="text" id="nome" name="nome" className="w-50 border-[1px] border-[#55133b] rounded"
                    value={nome} onChange={(e)=> setNome(e.target.value)}/>
                    {erroNome && <p className="text-[10px] bg-[#55133b] p-[3px] rounded text-[#F4F1E1]">{erroNome}</p>}
                    
                    <label htmlFor="email">Email</label>
                    <input type="email" id="email" name="email" className="w-50 border-[1px] border-[#55133b] rounded" 
                    value={email} onChange={(e)=> setEmail(e.target.value)}/>
                    {erroEmail && <p className="text-[10px] bg-[#55133b] p-[3px] rounded text-[#F4F1E1]">{erroEmail}</p>}

                    <label htmlFor="senha">Senha</label>
                    <input type="password" id="senha" name="senha" className="w-50 border-[1px] border-[#55133b] rounded" 
                    value={senha} onChange={(e)=> setSenha(e.target.value)} />
                    {erroSenha && <p className="text-[10px] bg-[#55133b] p-[3px] rounded text-[#F4F1E1]">{erroSenha}</p>}

                    <label htmlFor="confirmarSenha">Confirme a senha</label>
                    <input type="password" id="confirmarSenha" name="confirmarSenha" className="w-50 border-[1px] border-[#55133b] rounded" 
